...
type AuthMethod = 'email' | 'google' | 'github' | 'discord';
...
const authMethodConfig = {
  ...
  google: {
    ...
    variant: 'secondary' as const,
    className:
      'text-blue-700 bg-blue-50 hover:bg-blue-100 dark:text-blue-300 dark:bg-blue-900/30 dark:hover:bg-blue-900/50',
  },
  github: {
    label: 'GitHub',
    icon: null,
  ...
  discord: {
    ...
    variant: 'secondary' as const,
    className:
      'text-indigo-700 bg-indigo-50 hover:bg-indigo-100 dark:text-indigo-300 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50',
  },
};
...
export function AuthMethodBadge({
  ...
  const config = authMethodConfig[authMethod];
  const IconComponent = config.icon;

  const renderIcon = () => {
    if (authMethod === 'google') return <GoogleIcon />;
    if (authMethod === 'github') return <GitHubIcon />;
    if (authMethod === 'discord') return <DiscordIcon />;
    if (IconComponent) return <IconComponent className="h-3 w-3" />;
    return null;
  };
  ...
  return (
    <Badge
      ...
      className={cn(
        'text-xs font-medium flex items-center gap-1.5 px-2 py-1',
        config.className,
        className
      )}
    >
